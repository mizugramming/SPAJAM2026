...
### 現状
寿司職人のキャラクター、会輪のタイトル、コンセプト文を上部に表示し、同一の角丸パネル内で「つくる / 参加する」を切り替える。作成時は予定人数(3/4/5/6人)、参加時は6文字の部屋番号を入力して `ProfileInputScreen` へ画面遷移する。「AI寿司大将」という説明ラベルは表示しない。Room画面は小型スマホとキーボード表示に対応済み。実際の部屋作成/参加処理(`RoomService`呼び出し)と例外表示はプロフィール入力画面側で行う。
...
### 現状
参加者ごとの和紙・寿司皿風プロフィールカード(名前/区分/趣味/持ち込みテーマ/その参加者が選んだネタ一覧)を、横スワイプ(PageView)で表示する。アイコン・名前・区分はコンパクトな横並びで、前後カードの見切れ、中央カードのスケール演出、ページドット、現在人数表示、空データ表示まで実装済み。画面下部からタイトルへ戻ることができ、その際は端末内のセッション状態を初期化する。データはRoomProviderとConversationProviderから直接参照し、不明なTopic IDは安全に読み飛ばす。
...
### Done条件
参加者ごとのカード(区分/趣味/持ち込みTopic/その参加者が選んだTopic一覧)が、320px幅程度の小型スマホでも崩れず、横スワイプと縦スクロールで見やすく表示される。タイトルへ戻った後はRoom/Profile/Conversation履歴が初期化される。履歴不整合があっても画面がクラッシュしない。
