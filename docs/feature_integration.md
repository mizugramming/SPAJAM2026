...
`game_scene.dart` は共通 `PhoneViewport` 内の安全領域全体をゲームに渡す表示枠です。ゲーム中は缶・親分を重ねません。ゲーム中の結果選択・時刻早送りは右上の `DEMO` メニューからのみ使います。メニューの開閉と結果適用は `demo_page.dart` が担当します。通常のページ見出し・操作パネルをゲームの前後へ追加して領域を狭めたり、共通の画面幅・高さを変更したりしません。
...
各担当者は自分のゲームWidgetの `build` の中身を置き換え、確定時に `onCompleted(DuelGameResult.win)` などを一度呼びます。`Navigator` で結果画面へ直接移動せず、共通の結果処理へ戻します。毎秒の残り時間更新で親は再描画するため、タイマーやゲーム状態は `State` に保持して `build` で再初期化せず、終了時に `dispose` で解放します。上部の状態表示と右上の `DEMO` は共通側が重ねるので、重要なゲーム操作をその直下へ配置しません。
